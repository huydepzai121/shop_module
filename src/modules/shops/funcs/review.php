...
if ($showdata == 1) {
    ...
    if (!empty($array_data)) {
        ...
        $xtpl->assign('EMPTY', $nv_Lang->getModule('rate_empty'));
...
if ($timeout == 0 or NV_CURRENTTIME - $timeout > $difftimeout) {
    ...
    if (empty($sender)) {
        $contents = "NO_" . $nv_Lang->getModule('rate_empty_sender');
    } elseif (empty($rating)) {
        $contents = "NO_" . $nv_Lang->getModule('rate_empty_rating');
    } elseif ($pro_config['review_captcha'] and isset($fcaptcha) and !nv_capcha_txt($fcaptcha, $module_config[$module_name]['captcha_type'])) {
        $mess = ($module_config[$module_name]['captcha_type'] == 'recaptcha') ? $nv_Lang->getGlobal('securitycodeincorrect1') : $nv_Lang->getGlobal('securitycodeincorrect');
        $contents = "NO_" . $mess;
    }  else {
        ...
        if ($sth->execute()) {
            ...
            $contents = "OK_" . ($pro_config['review_check'] ? $nv_Lang->getModule('rate_success_queue') : $nv_Lang->getModule('rate_success_ok'));
        } else {
            $contents = "NO_" . $nv_Lang->getModule('rate_success_fail');
        }
        $nv_Request->set_Cookie($module_data . '_' . $op . '_' . $id, NV_CURRENTTIME);
    }
} else {
    $timeout = ceil(($difftimeout - NV_CURRENTTIME + $timeout) / 60);
    $timeoutmsg = sprintf($nv_Lang->getModule('detail_rate_timeout'), $timeout);
