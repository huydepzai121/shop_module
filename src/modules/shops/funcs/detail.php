...
if ($nv_Request->isset_request('get_available_attributes', 'post')) {
    ...
    // Lấy danh sách thuộc tính có sẵn trong kho
    $sql = 'SELECT DISTINCT t2.listgroup, t1.quantity, t1.price, t1.money_unit 
            FROM ' . $db_config['prefix'] . '_' . $module_data . '_warehouse_logs t1 
            INNER JOIN ' . $db_config['prefix'] . '_' . $module_data . '_warehouse_logs_group t2 ON t1.logid=t2.logid 
            WHERE t1.pro_id=' . $id_pro . ' AND t1.quantity > 0 
            ORDER BY t1.logid DESC';
    
    $query = $db->query($sql);
    while ($row = $query->fetch()) {
        $result['data'][] = array(
            'attribute_ids' => $row['listgroup'],
            'quantity' => $row['quantity'],
            'price' => $row['price'],
            'price_format' => number_format($row['price']),
...
if ($nv_Request->isset_request('check_quantity', 'post')) {
    ...
    $get_price = $nv_Request->get_int('get_price', 'post', 0);
    ...
    if (empty($listid)) {
        $result['message'] = $nv_Lang->getModule('detail_pro_number') . ': ' . $quantity . ' ' . $unit;
        nv_jsonOutput($result);
    }
    asort($listid);
    $listid_str = implode(',', $listid);

    $quantity = $db->query('SELECT quantity FROM ' . $db_config['prefix'] . '_' . $module_data . '_group_quantity WHERE pro_id = ' . $id_pro . ' AND listgroup="' . $listid_str . '"')->fetchColumn();
    $result['quantity'] = $quantity;
    
    // Lấy giá từ kho theo thuộc tính
    if ($get_price) {
        $sql = 'SELECT t1.price, t1.money_unit 
                FROM ' . $db_config['prefix'] . '_' . $module_data . '_warehouse_logs t1 
                INNER JOIN ' . $db_config['prefix'] . '_' . $module_data . '_warehouse_logs_group t2 ON t1.logid=t2.logid 
                WHERE t1.pro_id=' . $id_pro . ' AND t2.listgroup="' . $listid_str . '" 
                ORDER BY t1.logid DESC LIMIT 1';
        $warehouse_price = $db->query($sql)->fetch();
        
        if (!empty($warehouse_price)) {
            $result['price'] = array(
                'price' => $warehouse_price['price'],
                'price_format' => number_format($warehouse_price['price']),
                'money_unit' => $warehouse_price['money_unit']
            );
        }
    }
    ...
    if (empty($quantity)) {
        ...
        $result_query = $db->query('SELECT listgroup, quantity FROM ' . $db_config['prefix'] . '_' . $module_data . '_group_quantity WHERE pro_id = ' . $id_pro);
        while (list ($listgroup, $quantity) = $result_query->fetch(3)) {
        ...
        if ($sum == 0 or $count == sizeof($listid)) {
            $result['status'] = 'NO';
            $result['message'] = $nv_Lang->getModule('product_empty');
            nv_jsonOutput($result);
        } else {
            $result['status'] = 'NO';
            $result['message'] = $nv_Lang->getModule('detail_pro_number') . ': ' . $sum . ' ' . $unit;
            nv_jsonOutput($result);
        }
    } else {
        $result['message'] = $nv_Lang->getModule('detail_pro_number') . ': ' . $quantity . ' ' . $unit;
...
if (empty($data_content)) {
    ...
    redict_link($nv_Lang->getModule('detail_do_not_view'), $nv_Lang->getModule('redirect_to_back_shops'), $nv_redirect);
}
$id = $data_content['id'];
...
if (nv_user_in_groups($global_array_shops_cat[$catid]['groups_view'])) {
    ...
    if (!empty($data_content['tabs'])) {
        ...
        if ($pro_config['download_active']) {
            ...
            while ($row = $result->fetch()) {
                $row['filesize'] = !empty($row['filesize']) ? nv_convertfromBytes($row['filesize']) : $nv_Lang->getModule('download_file_unknown');
    ...
    // Lấy thuộc tính có sẵn trong kho
    ...
    $sql = 'SELECT DISTINCT t2.listgroup, t1.quantity, t1.price, t1.money_unit 
            FROM ' . $db_config['prefix'] . '_' . $module_data . '_warehouse_logs t1 
            INNER JOIN ' . $db_config['prefix'] . '_' . $module_data . '_warehouse_logs_group t2 ON t1.logid=t2.logid 
            WHERE t1.pro_id=' . $data_content['id'] . ' AND t1.quantity > 0 
            ORDER BY t1.logid DESC LIMIT 1';
    $result = $db->query($sql);
    
    if ($row = $result->fetch()) {
        $available_attribute = explode(',', $row['listgroup']);
        $warehouse_price = array(
            'price' => $row['price'],
            'price_format' => number_format($row['price']),
            'money_unit' => $row['money_unit'],
    ...
    $data_content['available_attribute'] = $available_attribute;
    $data_content['warehouse_price'] = $warehouse_price;
    ...
    $nv_redirect = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name;
    redict_link($nv_Lang->getModule('detail_no_permission'), $nv_Lang->getModule('redirect_to_back_shops'), $nv_redirect);
}
