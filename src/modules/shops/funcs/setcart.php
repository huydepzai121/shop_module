...
if (!is_numeric($num) or $num < 0) {
    $contents_msg = 'ERR_' . $nv_Lang->getModule('cart_set_err');
} else {
    if ($ac == 0) {
        if ($id > 0) {
            ...
            if ($num > $data_content['product_number'] and empty($pro_config['active_order_number'])) {
                $contents_msg = 'ERR_' . $nv_Lang->getModule('cart_set_err_num');
            } else {
                $update_cart = true;
                if (!isset($_SESSION[$module_data . '_cart'][$id . '_' . $group])) {
                    ...
                    if (($_SESSION[$module_data . '_cart'][$id . '_' . $group]['num'] + $num) > $data_content['product_number'] and empty($pro_config['active_order_number'])) {
                        $contents_msg = 'ERR_' . $nv_Lang->getModule('cart_set_err_num') . ': ' . $data_content['product_number'];
                        $update_cart = false;
                    } else {
                ...
                if ($update_cart) {
                    ...
                    $contents = sprintf($nv_Lang->getModule('set_cart_success'), $title);
        ...
        if ($id > 0) {
            ...
            if ($num > $data_content['product_number'] and empty($pro_config['active_order_number'])) {
                ...
                $contents_msg = 'OK_' . $nv_Lang->getModule('cart_set_ok') . $num;
            }
        }
    }
}
