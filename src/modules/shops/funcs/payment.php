...
$page_title = $nv_Lang->getModule('order_view');
...
if ($order_id > 0 and $checkss == md5($order_id . $global_config['sitekey'] . session_id())) {
    ...
    if (!empty($_SESSION[$module_data . '_order_info'])) {
        $nv_Lang->setModule('order_edit', $nv_Lang->getModule('order_unedit'));
    }
    ...
    if (intval($data['transaction_status']) == -1) {
        $intro_pay = $nv_Lang->getModule('payment_none_pay');
        ...
        $intro_pay = sprintf($nv_Lang->getModule('order_by_payment'));
    }
    if ($data['transaction_status'] == 4) {
        $data['transaction_name'] = $nv_Lang->getModule('history_payment_yes');
    } elseif ($data['transaction_status'] == 3) {
        $data['transaction_name'] = $nv_Lang->getModule('history_payment_cancel');
    } elseif ($data['transaction_status'] == 2) {
        $data['transaction_name'] = $nv_Lang->getModule('history_payment_check');
    } elseif ($data['transaction_status'] == 1) {
        $data['transaction_name'] = $nv_Lang->getModule('history_payment_send');
    } elseif ($data['transaction_status'] == 0) {
        $data['transaction_name'] = $nv_Lang->getModule('history_payment_no');
    } elseif ($data['transaction_status'] == -1) {
        $data['transaction_name'] = $nv_Lang->getModule('history_payment_wait');
    } else {
    ...
    if ($nv_Request->isset_request('worderid,wchecksum', 'get') and intval($data['transaction_status']) == 1 and isset($site_mods['wallet']) and file_exists(NV_ROOTDIR . '/modules/wallet/wallet.class.php')) {
        ...
        if ($transaction['order_id'] != $order_id) {
            redict_link('Data error!!!!', $nv_Lang->getModule('cart_back'), $link_back);
        }
        ...
        if (!$check) {
            redict_link($nv_Lang->getModule('payment_error_update'), $nv_Lang->getModule('cart_back'), $link_back);
        }
        ...
        if ($nv_transaction_status == 4) {
            ...
            $contents = redict_link($nv_Lang->getModule('payment_complete'), $nv_Lang->getModule('back_history'), $nv_redirect);

            $message = $nv_Lang->getModule('payment_complete');
        } else {
            $message = $nv_Lang->getModule('payment_notcomplete');
        }

        $nv_Cache->delMod($module_name);
        redict_link($message, $nv_Lang->getModule('cart_back'), $link_back);
    }
    ...
    if (isset($data['transaction_status']) and intval($data['transaction_status']) == 0 and $checksum == md5($order_id . $global_config['sitekey']) and isset($site_mods['wallet']) and file_exists(NV_ROOTDIR . '/modules/wallet/wallet.class.php')) {
        ...
        $data = [
            ...
            'order_object' => $nv_Lang->getModule('cart_title'), // Loại đối tượng được mua ví dụ: Ứng dụng, sản phẩm, giỏ hàng...
        ...
        if ($payment_info['status'] !== 'SUCCESS') {
            $link_back = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&order_id=' . $order_id . '&checkss=' . md5($order_id . $global_config['sitekey'] . session_id());
            redict_link($payment_info['message'], $nv_Lang->getModule('cart_back'), nv_url_rewrite($link_back));
        }
