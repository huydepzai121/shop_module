...
if ($nv_Request->isset_request('postorder', 'post')) {
    ...
    if (empty($data_order['order_name'])) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('order_name_err'),
            'input' => 'order_name'
        ]);
    }
    if (nv_check_valid_email($data_order['order_email']) != '') {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('order_email_err'),
            'input' => 'order_email'
        ]);
    }
    if (empty($data_order['order_phone'])) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('order_phone_err'),
            'input' => 'order_phone'
        ]);
    }
    if ($data_order['order_shipping'] and empty($data_order['shipping']['ship_name'])) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('order_shipping_name_err')
        ]);
    }
    if ($data_order['order_shipping'] and empty($data_order['shipping']['ship_phone'])) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('order_shipping_phone_err'),
            'input' => 'ship_phone'
        ]);
    }
    if ($data_order['order_shipping'] and empty($data_order['shipping']['ship_address_extend'])) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('shipping_address_extend_empty'),
    ...
    if ($data_order['order_shipping'] and empty($data_order['shipping']['ship_carrier_id'])) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('shipping_carrier_chose')
        ]);
    }
    if (! $nv_Request->get_int('check', 'post', 0)) {
        nv_jsonOutput([
            'error' => 1,
            'msg' => $nv_Lang->getModule('order_check_err')
        ]);
    }
    ...
    if ($i > 0) {
        ...
        if ($order_id > 0) {
            ...
            $nv_Lang->setModule('order_email_noreply', sprintf($nv_Lang->getModule('order_email_noreply'), $global_config['site_url'], $global_config['site_url']));
            $nv_Lang->setModule('order_email_thanks', sprintf($nv_Lang->getModule('order_email_thanks'), $global_config['site_url']));
            $nv_Lang->setModule('order_email_review', sprintf($nv_Lang->getModule('order_email_review'), $global_config['site_url'] . $review_url));
            ...
            $replace_data = [
                ...
                'review_url' => '<a href="' . $global_config['site_url'] . $data_order['review_url'] . '">' . $nv_Lang->getModule('content_here') . '</a>',
            ...
            if (file_exists($content_file)) {
                ...
                $content = $nv_Lang->getModule('order_payment_email');
            }
            ...
            $email_title = empty($order_info) ? $nv_Lang->getModule('order_email_title') : $nv_Lang->getModule('order_email_edit_title');
...
if ($action == 0) {
    $page_title = $nv_Lang->getModule('cart_check_cart');
    ...
    if (isset($_SESSION[$module_data . '_order_info']) and ! empty($_SESSION[$module_data . '_order_info'])) {
        $order_info = $_SESSION[$module_data . '_order_info'];
        $nv_Lang->setModule('order_submit_send', $nv_Lang->getModule('order_edit'));
    }
