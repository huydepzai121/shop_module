...
if ($nv_Request->isset_request('paypoint', 'get')) {
    ...
    if (empty($order_id) or $checkss != md5($client_info['session_id'] . $global_config['sitekey'] . $order_id)) {
        die('NO_' . $nv_Lang->getModule('payment_erorr'));
    } else {
        ...
        if (empty($order_data)) {
            ...
            die('NO_' . $nv_Lang->getModule('point_payment_error_money'));
        } else {
            ...
            nv_htmlOutput('OK_' . $nv_Lang->getModule('payment_complete'));
        }
    }
}
...
$data_content['money'] = number_format($data_content['money'], nv_get_decimals($pro_config['money_unit']));
