...
if (empty($user_info)) {
    die('NO_0_' . $nv_Lang->getModule('wishlist_guest'));
}
if (empty($id) or empty($ac)) {
    die('NO_0_' . $nv_Lang->getModule('wishlist_error'));
}
...
if ($ac == 'add') {
    ...
    if (!empty($wishlist)) {
        ...
        if (! in_array($id, $listid)) {
            ...
            die('NO_0_' . $nv_Lang->getModule('wishlist_exits'));
        }
    } else {
        $count = 1;
...
echo 'OK_' . $count . '_' . ($ac == 'add' ? $nv_Lang->getModule('wishlist_add_success') : $nv_Lang->getModule('wishlist_del_item_success'));
