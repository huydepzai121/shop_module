...
if (!function_exists('nv_view_product_price')) {
     ...
     * @return
     */
    function nv_block_config_product_price_blocks($module, $data_block)
    {
        global $nv_Lang;

        $html = '<div class="form-group">';
        $html .= '	<label class="control-label col-sm-6">' . $nv_Lang->getModule('price_begin') . '</label>';
        ...
        $html .= '</div>';
        $html .= '<div class="form-group">';
        $html .= '	<label class="control-label col-sm-6">' . $nv_Lang->getModule('price_end') . '</label>';
        $html .= '	<div class="col-sm-18"><input type="text" name="config_price_end" class="form-control w200" value="' . $data_block['price_end'] . '"/></div>';
        $html .= '</div>';
        $html .= '<div class="form-group">';
        $html .= '	<label class="control-label col-sm-6">' . $nv_Lang->getModule('price_step') . '</label>';
    ...
    function numoney_to_strmoney($money, $mod_file)
    {
        ...
        if ($money > 1000 and $money < 1000000) {
            $money = $money / 1000;
            return $money . ' ' . $nv_Lang->getModule('money_thousand');
        } elseif ($money >= 1000000) {
            $money = $money / 1000000;
            return $money . ' ' . $nv_Lang->getModule('money_million');
        }
        return $money;
    }
     ...
     * @return
     */
    function nv_block_config_product_price_blocks_submit($module)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_view_product_price($block_config)
    {
        ...
        while (true) {
            ...
            if ($val < $block_config['price_end']) {
                ...
                $title = '<span class="label label-warning">' . $nv_Lang->getModule('price2_over') . ' ' . numoney_to_strmoney($val, $mod_file) . '</span>';
