...
if (!function_exists('nv_filter_product')) {
     ...
     * @return
     */
    function nv_block_config_filter_product($module, $data_block)
    {
        global $nv_Lang;

        $html = '';
        $html .= "<div class=\"form-group\">";
        $html .= "	<label class=\"control-label col-sm-6\">" . $nv_Lang->getModule('group_price') . "</label>";
     ...
     * @return
     */
    function nv_block_config_filter_product_submit($module)
    {
        global $nv_Request;
        $return = [];
        $return['config'] = [];
    ...
    {
        global $nv_Request, $module_name, $module_info, $module_file, $module_upload, $db, $module_data, $db_config, $id, $catid, $pro_config, $global_config, $global_array_group, $global_array_shops_cat, $pro_config, $catid, $crypt, $nv_Lang;
            ...
            if (!empty($price_value)) {
                ...
                foreach ($price_value as $value) {
                    ...
                    if (isset($price_value[$i + 1])) {
                        ...
                        $price = [
                            'price_key' => $price_key,
                            'price_value' => number_format($price1) . ' - ' . number_format($price2),
                        ...
                        $price = [
                            'price_key' => $price_key,
                            'price_value' => sprintf($nv_Lang->getModule('filter_price_from'), number_format($price1)),
                            'checked' => in_array($price_key, $group_price) ? 'checked="checked"' : ''
                        ];
                    }
