...
{
    global $db, $module_name, $module_data, $title, $db_config, $nv_Lang;
...
function nv_link_edit_page($id)
{
    ...
    $link = "<a class=\"btn btn-default btn-xs\" href=\"" . NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . "=" . $module_name . '&amp;' . NV_OP_VARIABLE . "=content&amp;id=" . $id . "\" title=\"" . $nv_Lang->getGlobal('edit') . "\"><i class=\"fa fa-edit\"></i></a>";
    return $link;
}
...
{
    global $module_name, $nv_Lang;
    $link = "<a class=\"btn btn-danger btn-xs\" href=\"javascript:void(0);\" onclick=\"nv_del_content(" . $id . ", '" . md5($id . session_id()) . "','" . NV_BASE_ADMINURL . "')\" title=\"" . $nv_Lang->getGlobal('delete') . "\"><i class=\"fa fa-trash-o\"></i></a>";
    return $link;
}
