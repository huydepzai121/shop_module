 ...
 * @return
 */
function nv_get_price($pro_id, $currency_convert, $number = 1, $per_pro = false, $module = '', $listid = '')
{
    ...
    // Lấy giá từ kho theo thuộc tính nếu có
    if (!empty($listid)) {
        $listid = is_array($listid) ? $listid : explode(',', $listid);
        asort($listid);
        $listid = implode(',', $listid);
        
        $sql = 'SELECT t1.price, t1.sale_price, t1.money_unit 
                ...
                WHERE t1.pro_id=' . $pro_id . ' AND t2.listgroup="' . $listid . '" 
                ORDER BY t1.logid DESC LIMIT 1';
        $warehouse_price = $db->query($sql)->fetch();
        
        if (!empty($warehouse_price)) {
            $price = $warehouse_price['price'];
            $product['money_unit'] = $warehouse_price['money_unit'];
            if (!empty($warehouse_price['sale_price'])) {
                $product['saleprice'] = $warehouse_price['sale_price'];
            }
        }
    }
    ...
    $return['price_format'] = number_format($price, $decimals); // Giá sản phẩm đã format

    $return['discount'] = $discount; // Số tiền giảm giá sản phẩm chưa format
    $return['discount_format'] = number_format($discount, $decimals); // Số tiền giảm giá sản phẩm đã format
    $return['discount_percent'] = $discount_unit == '%' ? $discount_percent : number_format($discount_percent, $decimals); // Giảm giá theo phần trăm
    $return['discount_unit'] = $discount_unit; // Đơn vị giảm giá

    $return['sale'] = $price - $discount; // Giá bán thực tế của sản phẩm
    $return['sale_format'] = number_format($return['sale'], $decimals); // Giá bán thực tế của sản phẩm đã format
