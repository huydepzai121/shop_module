...
{
    global $module_info, $module_name, $module_file, $pro_config, $array_wishlist_id, $global_array_shops_cat, $global_array_group, $my_head, $nv_Lang;
...
{
    global $module_info, $module_file, $global_array_shops_cat, $module_name, $nv_Lang;
    ...
    if (!empty($array_data)) {
        foreach ($array_data as $data_row) {
            if ($data_row['num_pro'] > 0) {
                ...
                if (!empty($data_row['catid']) and !empty($global_array_shops_cat[$data_row['catid']]['image'])) {
...
{
    global $module_info, $module_name, $module_file, $pro_config, $array_wishlist_id, $global_array_shops_cat, $global_array_blockcat, $my_head, $nv_Lang;
...
{
    global $module_info, $module_file, $module_name, $module_upload, $pro_config, $global_config, $global_array_group, $array_wishlist_id, $client_info, $global_array_shops_cat, $meta_property, $user_info, $discounts_config, $my_head, $my_footer, $nv_Lang;
    ...
    $xtpl->assign('GLANG', \NukeViet\Core\Language::$lang_module);
    $xtpl->assign('LANG', \NukeViet\Core\Language::$lang_module);
    ...
    if (!empty($data_content)) {
        ...
        $xtpl->assign('DATE_UP', $nv_Lang->getModule('detail_dateup') . ' ' . nv_date('d-m-Y h:i:s A', $data_content['publtime']));
        ...
        // Sử dụng giá từ kho nếu có
        if (!empty($data_content['warehouse_price'])) {
            $price = array(
                'price' => $data_content['warehouse_price']['price'],
                'price_format' => $data_content['warehouse_price']['price_format'],
                'unit' => $data_content['warehouse_price']['money_unit']
            );
            
            // Truyền thuộc tính có sẵn vào template để tự động chọn
            $xtpl->assign('AVAILABLE_ATTRIBUTE', json_encode($data_content['available_attribute']));
            $xtpl->parse('main.available_attribute');
        } else {
            // Lấy giá mặc định nếu không có trong kho
            ...
            if ($parent_info['in_order']) {
                ...
                if (!empty($subid)) {
                    foreach ($subid as $sub_gr_id) {
                        ...
                        if (!empty($sub_info['image']) and file_exists(NV_ROOTDIR . '/' . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $sub_info['image'])) {
                            $sub_info['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $sub_info['image'];
                        } else {
                            $sub_info['image'] = '';
                        }
                        ...
                        if ($sub_info['in_order']) {
                            ...
                            if (!empty($sub_info['image'])) {
                                $xtpl->parse('main.group.items.loop.image');
                            }
    ...
    if ($global_array_shops_cat[$data_content['listcatid']]['typeprice'] == 2) {
        ...
        if (!empty($price_config) and sizeof($price_config) > 1) {
            ...
            foreach ($price_config as $items) {
                ...
                $items['price'] = number_format($items['price'], nv_get_decimals($pro_config['money_unit']));
    ...
    if ($pro_config['active_price'] == '1') {
        if ($data_content['showprice'] == '1' && !empty($data_content['product_price'])) {
            if (isset($price['discount_percent']) && $price['discount_percent'] > 0) {
...
{
    global $module_info, $module_file, $global_config, $module_name, $pro_config, $global_array_shops_cat, $nv_Lang;
    ...
    if (!empty($data_content)) {
        ...
        $xtpl->assign('TITLE', $data_content[NV_LANG_DATA . '_title']);
        $xtpl->assign('NUM_VIEW', $data_content['hitstotal']);
        $xtpl->assign('DATE_UP', $nv_Lang->getModule('detail_dateup') . date(' d-m-Y ', $data_content['addtime']) . $nv_Lang->getModule('detail_moment') . date(" H:i'", $data_content['addtime']));
 ...
 * @return
 */
function cart_product($data_content, $coupons_code, $order_info, $array_error_number, $data_order, $shipping_data)
{
    global $module_info, $module_config, $module_file, $module_name, $pro_config, $money_config, $global_array_group, $global_array_shops_cat, $nv_Lang, $module_data;
    ...
    if (!empty($data_content)) {
        ...
        foreach ($data_content as $data_row) {
            ...
            $xtpl->assign('homeimgthumb', $data_row['homeimgthumb']);
        ...
        if ($pro_config['point_active']) {
            ...
            if (defined('NV_IS_USER')) {
                $xtpl->assign('point_note', sprintf($nv_Lang->getModule('point_cart_note_user'), $point_total));
            } else {
                ...
                $xtpl->assign('point_note', sprintf($nv_Lang->getModule('point_cart_note_guest'), $point_total, $login));
            }
    ...
    $xtpl->assign('price_total', number_format($price_total, nv_get_decimals($pro_config['money_unit'])));
    ...
    if (!empty($order_info)) {
        $xtpl->assign('EDIT_ORDER', sprintf($nv_Lang->getModule('cart_edit_warning'), $order_info['order_url'], $order_info['order_code'], $order_info['order_edit']));
    ...
    if ($module_config[$module_name]['use_shipping']) {
        ...
        $array_yes_no = array(
            $nv_Lang->getModule('shipping_offline'),
            $nv_Lang->getModule('shipping_online')
        );
...
{
    global $module_info, $module_config, $module_data, $module_file, $module_name, $pro_config, $money_config, $global_array_group, $shipping_data, $nv_Lang;
    ...
    $xtpl->assign('price_coupons', number_format($total_coupons, nv_get_decimals($pro_config['money_unit'])));
    $xtpl->assign('price_total', number_format($price_total - $total_coupons, nv_get_decimals($pro_config['money_unit'])));
    $xtpl->assign('unit_config', $money_config[$pro_config['money_unit']]['symbol']);
    $xtpl->assign('weight_unit', $pro_config['weight_unit']);
    $xtpl->assign('DATA', $data_order);
    $xtpl->assign('LINK_CART', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=cart');
...
{
    global $module_info, $module_data, $module_file, $global_config, $module_name, $pro_config, $money_config, $global_array_group, $client_info, $array_location, $array_shops, $nv_Lang;

    $money = $point * $pro_config['point_conversion'];
    $money = number_format($money, nv_get_decimals($pro_config['money_unit']));
    $nv_Lang->setModule('point_payment_info', sprintf($nv_Lang->getModule('point_payment_info'), $point, $money, $pro_config['money_unit']));
    ...
    foreach ($data_pro as $pdata) {
        ...
        $xtpl->assign('product_price_total', number_format($pdata['product_price'] * $pdata['product_number'], nv_get_decimals($pro_config['money_unit'])));
    ...
    $xtpl->assign('order_coupons', (!empty($data_content['coupons']['amount']) ? number_format($data_content['coupons']['amount'], nv_get_decimals($pro_config['money_unit'])) : ''));
...
{
    global $module_info, $module_file, $pro_config, $money_config, $global_array_group, $nv_Lang;
    ...
    if ($data_content['transaction_status'] == 4) {
        $payment = $nv_Lang->getModule('history_payment_yes');
    } elseif ($data_content['transaction_status'] == 3) {
        $payment = $nv_Lang->getModule('history_payment_cancel');
    } elseif ($data_content['transaction_status'] == 2) {
        $payment = $nv_Lang->getModule('history_payment_check');
    } elseif ($data_content['transaction_status'] == 1) {
        $payment = $nv_Lang->getModule('history_payment_send');
    } elseif ($data_content['transaction_status'] == 0) {
        $payment = $nv_Lang->getModule('history_payment_no');
    } elseif ($data_content['transaction_status'] == -1) {
        $payment = $nv_Lang->getModule('history_payment_wait');
    } else {
        $payment = 'ERROR';
    }
...
{
    global $module_info, $module_file, $module_name, $pro_config, $money_config, $nv_Lang;
    ...
    foreach ($data_content as $data_row) {
        ...
        $xtpl->assign('history_total', number_format($data_row['order_total'], nv_get_decimals($pro_config['money_unit'])));
        ...
        if ($data_row['transaction_status'] == 4) {
            $history_payment = $nv_Lang->getModule('history_payment_yes');
        } elseif ($data_row['transaction_status'] == 3) {
            $history_payment = $nv_Lang->getModule('history_payment_cancel');
        } elseif ($data_row['transaction_status'] == 2) {
            $history_payment = $nv_Lang->getModule('history_payment_check');
        } elseif ($data_row['transaction_status'] == 1) {
            $history_payment = $nv_Lang->getModule('history_payment_send');
        } elseif ($data_row['transaction_status'] == 0) {
            $history_payment = $nv_Lang->getModule('history_payment_no');
        } elseif ($data_row['transaction_status'] == -1) {
            $history_payment = $nv_Lang->getModule('history_payment_wait');
        } else {
...
{
    global $module_name, $module_info, $module_file, $module_name, $nv_Cache, $db_config, $module_data, $crypt, $nv_Lang;
...
{
    global $module_file, $module_info, $global_array_shops_cat, $pro_config, $global_config, $nv_Lang;
...
{
    global $module_file, $module_info, $pro_config, $global_array_group, $my_head, $nv_Lang;
...
{
    global $module_info, $module_data, $module_file, $pro_config, $op, $nv_Lang;
    ...
    if (!empty($data_content)) {
        ...
        $data_content['date_end'] = !empty($data_content['date_end']) ? nv_date('d/m/Y', $data_content['date_end']) : $nv_Lang->getModule('coupons_end_time_ulimit');
...
{
    global $module_info, $module_name, $module_data, $module_file, $pro_config, $op, $user_info, $global_config, $module_config, $nv_Lang;
...
{
    global $module_info, $module_name, $module_data, $module_file, $pro_config, $op, $nv_Lang;
    ...
    if (!empty($data_content['files'])) {
        ...
        foreach ($data_content['files'] as $files) {
            ...
            if (!nv_user_in_groups($files['download_groups'])) {
                $xtpl->assign('NOTE', $nv_Lang->getModule('download_file_no'));
                $xtpl->parse('main.files_content.loop.disabled');
            } else {
                $xtpl->assign('NOTE', $nv_Lang->getModule('download_file'));
            }
        ...
        if ($login > 0) {
            ...
            $xtpl->assign('DOWNLOAD_LOGIN', '<a title="' . $nv_Lang->getGlobal('loginsubmit') . '" href="' . $link_login . '">' . $nv_Lang->getModule('download_login') . '</a>');
...
{
    global $module_info, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid, $global_array_shops_cat, $nv_Lang;
    ...
    if (!empty($array_data)) {
        ...
        foreach ($array_data as $data_row) {
            ...
            if ($pro_config['active_wishlist']) {
                ...
                if (!empty($data_row['yeuthich'])) {
                    $xtpl->parse('main.loop.wishlist.yeuthich');
                } else {
                    $xtpl->parse('main.loop.wishlist.no_yeuthich');
            ...
            if ($pro_config['point_active'] and $global_array_shops_cat[$data_row['listcatid']]['cat_allow_point'] and !empty($global_array_shops_cat[$data_row['listcatid']]['cat_number_point'])) {
                ...
                $xtpl->assign('point_note', sprintf($nv_Lang->getModule('point_product_note'), $global_array_shops_cat[$data_row['listcatid']]['cat_number_point']));
...
{
    global $module_info, $module_file, $module_upload, $module_name, $pro_config, $array_displays, $array_wishlist_id, $op, $global_array_shops_cat, $global_array_group, $my_head, $page, $nv_Lang;
    ...
    if (!empty($pro_config['show_displays']) and ($op == 'group' or $global_array_shops_cat[$data_content['id']]['viewcat'] != 'view_home_cat')) {
        ...
        $array_viewtype = array(
            'viewgrid' => array(
                'title' => $nv_Lang->getModule('view_page_gird'),
                'icon' => 'th-large'
            ),
            'viewlist' => array(
                'title' => $nv_Lang->getModule('view_page_list'),
                'icon' => 'th-list'
            )
        );
...
{
    global $module_info, $module_file, $op, $page_title, $pro_config, $array_displays, $nv_Lang;
