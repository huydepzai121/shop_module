...
if ($groupid > 0) {
    ...
    if (intval($check_parentid) > 0) {
        $contents = "ERR_GROUP_" . sprintf($nv_Lang->getModule('delgroup_msg_group'), $check_parentid);
    } else {
        ...
        if (intval($check_rows) > 0) {
            if ($delallcheckss == md5($groupid . session_id() . $global_config['sitekey'])) {
                ...
                if (empty($delgroupandrows) and empty($movegroup)) {
                    ...
                    $array_group_list[0] = $nv_Lang->getModule('delgroup_no_group');
                    ...
                    $xtpl->assign('INFO', sprintf($nv_Lang->getModule('delgroup_msg_rows_select'), $title, $check_rows));
                ...
                $contents = "ERR_ROWS_" . $groupid . "_" . md5($groupid . session_id() . $global_config['sitekey']) . "_" . sprintf($nv_Lang->getModule('delgroup_msg_rows'), $check_rows);
            }
        }
    }
    ...
    if ($contents == "NO_" . $groupid) {
        ...
        if ($db->exec($sql)) {
            ...
            nv_insert_logs(NV_LANG_DATA, $module_name, $nv_Lang->getModule('delgroupandrows'), $title, $admin_info['userid']);
        }
