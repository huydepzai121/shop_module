...
$contents = $nv_Lang->getModule('order_submit_pay_error');
...
if ($save == 1) {
    ...
    if ($action == 'unpay') {
        ...
        $contents = $nv_Lang->getModule('order_submit_unpay_ok');
        ...
        $replace_data = array(
            ...
            'order_total' => number_format($data_content['order_total']),
            'unit_total' => $data_content['unit_total'],
            'dateup' => nv_date("d-m-Y", $data_content['order_time']),
            'moment' => nv_date("H:i", $data_content['order_time']),
            'review_url' => '<a href="' . $global_config['site_url'] . $review_url . '">' . $nv_Lang->getModule('here') . '</a>',
        ...
        if (file_exists($content_file)) {
            ...
            if (empty($content)) {
                $content = $nv_Lang->getModule('order_email_payment');
            }
        ...
        ), $data_content['order_email'], sprintf($nv_Lang->getModule('document_payment_email_order_payment'), $module_info['custom_title'], $data_content['order_code']), $email_contents);

        $contents = $nv_Lang->getModule('order_submit_pay_ok');
