...
$page_title = $nv_Lang->getModule('cat_title');
...
$data['typeprice'] = 1;
$data['parent_title'] = '';
$data['tag_description'] = ''; // Thêm giá trị mặc định cho tag_description
...
if ($nv_Request->isset_request('mod', 'post, get')) {
    ...
    if ($mod == 'weight' and $new_vid > 0) {
        $sql = 'SELECT catid FROM ' . $table_name . ' WHERE catid!=' . $catid . ' AND parentid=' . $parentid . ' ORDER BY weight ASC';
        $result = $db->query($sql);
        ...
        while ($row = $result->fetch()) {
            ...
            $sql = 'UPDATE ' . $table_name . ' SET weight=' . $weight . ' WHERE catid=' . intval($row['catid']);
            $db->query($sql);
        }

        $sql = 'UPDATE ' . $table_name . ' SET weight=' . $new_vid . ' WHERE catid=' . $catid;
        $db->query($sql);
        ...
        $sql = 'UPDATE ' . $table_name . ' SET inhome=' . $new_vid . ' WHERE catid=' . $catid;
        $db->query($sql);
        $nv_Cache->delMod($module_name);
        nv_jsonOutput([
            'status' => 'success',
        ...
        $sql = 'UPDATE ' . $table_name . ' SET numlinks=' . $new_vid . ' WHERE catid=' . $catid;
        $db->query($sql);
        $nv_Cache->delMod($module_name);
        nv_jsonOutput([
            'status' => 'success',
    ...
    } elseif ($mod == 'viewcat' and $new_vid >= 0) {
        ...
        $sql = 'UPDATE ' . $table_name . ' SET viewcat=' . $db->quote($viewcat) . ' WHERE catid=' . $catid;
        $db->query($sql);
        $nv_Cache->delMod($module_name);
        nv_jsonOutput([
            'status' => 'success',
        ...
        $sql = 'UPDATE ' . $table_name . ' SET newday=' . $new_vid . ' WHERE catid=' . $catid;
        $db->query($sql);
        $nv_Cache->delMod($module_name);
        nv_jsonOutput([
            'status' => 'success',
...
if ($nv_Request->isset_request('delete_cat', 'post,get')) {
    $catid = $nv_Request->get_int('catid', 'post,get', 0);
    ...
    list($catid, $parentid, $title) = $db->query('SELECT catid, parentid, ' . NV_LANG_DATA . '_title FROM ' . $table_name . ' WHERE catid=' . $catid)->fetch(3);

    // Kiểm tra có danh mục con
    $check_subcats = $db->query('SELECT COUNT(*) FROM ' . $table_name . ' WHERE parentid = ' . $catid)->fetchColumn();
    if ($check_subcats > 0) {
        nv_jsonOutput([
            'error' => 1,
    ...
    if ($check_products > 0) {
        if (empty($action)) {
            ...
            $sql = 'SELECT catid, ' . NV_LANG_DATA . '_title title FROM ' . $table_name . ' WHERE catid !=' . $catid . ' ORDER BY sort ASC';
            $result = $db->query($sql);
            $array_cats = [];
            while ($row = $result->fetch()) {
                $array_cats[] = $row;
    ...
    // Xóa danh mục
    $db->query('DELETE FROM ' . $table_name . ' WHERE catid=' . $catid);
    nv_fix_cat_order();
    $nv_Cache->delMod($module_name);
...
if (!empty($savecat)) {
    ...
    if ($data['title'] == '') {
        nv_jsonOutput(array(
            'error' => 1,
            'msg' => $nv_Lang->getModule('error_cat_name')
        ));
    }
    ...
    if ($check_alias) {
        nv_jsonOutput(array(
            'error' => 1,
            'msg' => $nv_Lang->getModule('error_unique_alias')
        ));
    }
                    ...
                    if ($parentid != 0) {
                        ...
                        while ($row_group = $result_group_items->fetch()) {
                            ...
                            if ($row_check['is_require'] == 1) {
                                nv_jsonOutput(array(
                                    'error' => 1,
                                    'msg' => sprintf($nv_Lang->getModule('error_cat_update'), $row_check[NV_LANG_DATA . '_title'])
                                ));
                            }
...
if ($data['parentid']) {
    $nv_Lang->setModule('setting_group_price_space_note_cat', sprintf($nv_Lang->getModule('setting_group_price_space_note_cat_1'), $data['parent_title']));
} else {
    ...
    $nv_Lang->setModule('setting_group_price_space_note_cat', sprintf($nv_Lang->getModule('setting_group_price_space_note_cat_0'), $URL_SETTING_GR_PRICE));
}
...
// Chuẩn bị dữ liệu nhóm xem
$groups_view = explode(',', $data['groups_view']);
$array_groups_view = [];
foreach ($groups_list as $_group_id => $_title) {
    $array_groups_view[] = array(
        'value' => $_group_id,
...
$array_typeprice = array(
    $nv_Lang->getModule('typeprice_none'),
    $nv_Lang->getModule('config_discounts'),
    $nv_Lang->getModule('typeprice_per_product')
);
$array_typeprice_list = [];
foreach ($array_typeprice as $key => $value) {
    $array_typeprice_list[] = array(
        'key' => $key,
        'value' => $value,
        'checked' => $data['typeprice'] == $key ? 'checked="checked"' : ''
    );
}
...
$array_viewdescription = [];
for ($i = 0; $i <= 2; $i++) {
    $array_viewdescription[] = array(
        'value' => $i,
        'checked' => $data['viewdescriptionhtml'] == $i ? ' checked="checked"' : '',
        'title' => $nv_Lang->getModule('content_bodytext_display_' . $i)
    );
}

// Gán dữ liệu cho template
$template = get_tpl_dir([$global_config['module_theme'], $global_config['admin_theme']], 'admin_future', '/modules/' . $module_file . '/cat.tpl');
...
$tpl->assign('CAPTION', ($data['catid'] > 0) ? $nv_Lang->getModule('edit_cat') : $nv_Lang->getModule('add_cat'));
$tpl->assign('UPLOAD_CURRENT', $currentpath);
$tpl->assign('FORM_ACTION', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;catid=' . $data['catid'] . '&amp;parentid=' . $data['parentid']);
$tpl->assign('CAT_LIST', shops_show_cat_list($data['parentid']));
$tpl->assign('DESCRIPTIONHTML', $descriptionhtml);
$tpl->assign('getalias', true);
// Gán các mảng dữ liệu
$tpl->assign('parent_loop', $array_parent_list);
$tpl->assign('groups_view', $array_groups_view);
$tpl->assign('typeprice_loop', $array_typeprice_list);
$tpl->assign('viewdescriptionhtml', $array_viewdescription);
if (!empty($array_cat_form)) {
    $tpl->assign('cat_form', [
        'loop' => $array_cat_form
    ]);
}
...
if ($pro_config['point_active']) {
    $tpl->assign('point', true);
}

$tpl->assign('main', true);
$contents = $tpl->fetch('cat_add.tpl');
