...
$page_title = sprintf($nv_Lang->getModule('seller_list'), $pro_info['title']);
...
while (list($order_name, $order_email, $order_phone, $order_address, $unit_total, $order_time, $num, $price) = $sth->fetch(3)) {
    ...
    $xtpl->assign('ROW', array(
        ...
        'num' => $num,
        'price' => number_format($price, nv_get_decimals($unit_total)),
...
$array_total['price'] = number_format($array_total['price'], nv_get_decimals($array_total['pro_unit']));
$xtpl->assign('TOTAL', $array_total);
$xtpl->assign('C_REPORT', sprintf($nv_Lang->getModule('customer_report_display'), $i, $num_items));
