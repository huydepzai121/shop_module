...
function getgroup_ckhtml($subgroupid_i, $array_groupid_in_row)
{
    ...
    if (!empty($subgroupid_i)) {
        foreach ($subgroupid_i as $groupid_i) {
            ...
            $contents_temp .= '<label class="col-sm-2"><input type="checkbox" name="groupids[]" value="' . $groupid_i . '"' . $ch . ' />' . $image . '&nbsp;' . $data_group['title'] . '</label>';
        }
    }
...
if (empty($cid)) {
    $cid = $db->query('SELECT catid FROM ' . $db_config['prefix'] . '_' . $module_data . '_catalogs WHERE parentid = 0 ORDER BY weight ASC LIMIT 1')->fetchColumn();
}
