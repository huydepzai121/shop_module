        ...
        if ($value != '') {
            if ($row['field_type'] == 'number') {
                ...
                if (!preg_match($pattern, $value)) {
                    $error = sprintf($nv_Lang->getModule('field_match_type_error'), $row['title']);
                } else {
                    $value = ($number_type == 1) ? intval($value) : floatval($value);

                    if ($value < $row['min_length'] or $value > $row['max_length']) {
                        $error = sprintf($nv_Lang->getModule('field_min_max_value'), $row['title'], $row['min_length'], $row['max_length']);
                    }
                }
                ...
                if (preg_match("/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/", $value, $m)) {
                    ...
                    if ($value < $row['min_length'] or $value > $row['max_length']) {
                        $error = sprintf($nv_Lang->getModule('field_min_max_value'), $row['title'], date('d/m/Y', $row['min_length']), date('d/m/Y', $row['max_length']));
                    }
                } else {
                    $error = sprintf($nv_Lang->getModule('field_match_type_error'), $row['title']);
                }
                ...
                if ($strlen < $row['min_length'] or $strlen > $row['max_length']) {
                    $error = sprintf($nv_Lang->getModule('field_min_max_error'), $row['title'], $row['min_length'], $row['max_length']);
                }
