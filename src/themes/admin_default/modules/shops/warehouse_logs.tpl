		...
		<div class="row">
			<div class="col-xs-6">
				<div class="form-group">
					<input type="text" class="form-control" value="{$SEARCH.keywords}" name="keywords" placeholder="{$LANG->getModule('search_key')}" />
				</div>
			</div>
			<div class="col-sm-6">
				<div class="form-group">
					<div class="input-group">
						<input type="text" class="form-control" name="from" id="from" value="{$SEARCH.from}" readonly="readonly" placeholder="{$LANG->getModule('date_from')}">
			...
			<div class="col-sm-6">
				<div class="form-group">
					<div class="input-group">
						<input type="text" class="form-control" name="to" id="to" value="{$SEARCH.to}" readonly="readonly" placeholder="{$LANG->getModule('date_to')}">
						<span class="input-group-btn">
			...
			<div class="col-sm-6">
				<div class="form-group">
					<input type="submit" class="btn btn-primary" value="{$LANG->getModule('search')}" />
					<input type="button" class="btn btn-danger" value="{$LANG->getModule('reset')}" onclick="$('#search_form').clearForm();" />
				</div>
			</div>
		</div>
	</form>
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<thead>
			<tr>
				...
				<th>{$LANG->getModule('warehouse_num_pro')}</th>
				<th>{$LANG->getModule('order_total')} ({$MONEY_UNIT})</th>
				<th class="text-center">{$LANG->getModule('warehouse_time')}</th>
			</tr>
		</thead>
		<tbody>
			<!-- BEGIN: loop -->
